...
class CnnHeadPoseEstimator:
    ...
    def __init__(self, YawFilePath, PitchFilePath):
        ...
        self._sess = tf.compat.v1.Session()  # (config=tf.ConfigProto(gpu_options=gpu_options))

        tf.compat.v1.train.Saver(({"conv1y_w": self.hy_conv1_weights, "conv1y_b": self.hy_conv1_biases,
        ...
        tf.compat.v1.train.Saver(({"conv1p_w": self.hp_conv1_weights, "conv1p_b": self.hp_conv1_biases,
    ...
    def _init_yaw_(self, YawFilePath):
        ...
        self.tf_yaw_input_vector = tf.compat.v1.placeholder(tf.float32, shape=(40, 40))

        # Variables.
        # Conv layer
        #[patch_size, patch_size, num_channels, depth]
        self.hy_conv1_weights = tf.Variable(tf.random.truncated_normal([3, 3, 1, 64], stddev=0.01))
        self.hy_conv1_biases = tf.Variable(tf.zeros([64]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hy_conv2_weights = tf.Variable(tf.random.truncated_normal([3, 3, 64, 128], stddev=0.01))
        self.hy_conv2_biases = tf.Variable(tf.random.normal(shape=[128]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hy_conv3_weights = tf.Variable(tf.random.truncated_normal([3, 3, 128, 256], stddev=0.01))  # was[3, 3, 128, 256]
        self.hy_conv3_biases = tf.Variable(tf.random.normal(shape=[256]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hy_conv4_weights = tf.Variable(tf.random.truncated_normal([3, 3, 256, 512], stddev=0.01))  # was[3, 3, 128, 256]
        self.hy_conv4_biases = tf.Variable(tf.random.normal(shape=[512]))

        # Dense layer
        #[ 5*5 * previous_layer_out , num_hidden] wd1
        # here 5*5 is the size of the image after pool reduction (divide by
        # half 3 times)
        self.hy_dense1_weights = tf.Variable(tf.random.truncated_normal([10*10*512, 4096], stddev=0.01))  # was [5*5*256, 1024]
        self.hy_dense1_biases = tf.Variable(tf.random.normal(shape=[4096]))
        # Dense layer
        #[ , num_hidden] wd2
        self.hy_dense2_weights = tf.Variable(tf.random.truncated_normal([4096, 4096], stddev=0.01))
        self.hy_dense2_biases = tf.Variable(tf.random.normal(shape=[4096]))
        # Output layer
        self.hy_out_weights = tf.Variable(tf.random.truncated_normal([4096, self._num_labels], stddev=0.01))
        ...
        def model(data):
            ...
            # Convolution Layer 1
            conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, filters=self.hy_conv1_weights,
                strides=[1, 1, 1, 1], padding='SAME'), self.hy_conv1_biases))
            if(DEBUG == True):
                print("SHAPE conv1: " + str(conv1.get_shape()))
            # Max Pooling (down-sampling)
            pool1 = tf.nn.max_pool2d(input=conv1, ksize=[1, 2, 2, 1],
            ...
            # Convolution Layer 2
            conv2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm1, filters=self.hy_conv2_weights,
                strides=[1, 1, 1, 1], padding='SAME'), self.hy_conv2_biases))
            if(DEBUG == True):
                print("SHAPE conv2: " + str(conv2.get_shape()))
            # Max Pooling (down-sampling)
            pool2 = tf.nn.max_pool2d(input=conv2, ksize=[1, 2, 2, 1],
                strides=[1, 2, 2, 1], padding='SAME')
            if(DEBUG == True):
            ...
            # Convolution Layer 3
            conv3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm2, filters=self.hy_conv3_weights,
            ...
            # Convolution Layer 4
            conv4 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv3, filters=self.hy_conv4_weights,
    ...
    def _init_pitch_(self, PitchFilePath):
        num_labels_pitch = 8
        # Input data [batch_size, image_size, image_size, channels]
        self.tf_pitch_input_vector = tf.compat.v1.placeholder(tf.float32, shape=(40, 40))

        # Variables.
        # Conv layer
        #[patch_size, patch_size, num_channels, depth]
        self.hp_conv1_weights = tf.Variable(tf.random.truncated_normal([3, 3, 1, 64], stddev=0.01))
        self.hp_conv1_biases = tf.Variable(tf.zeros([64]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hp_conv2_weights = tf.Variable(tf.random.truncated_normal([3, 3, 64, 128], stddev=0.01))
        self.hp_conv2_biases = tf.Variable(tf.random.normal(shape=[128]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hp_conv3_weights = tf.Variable(tf.random.truncated_normal([3, 3, 128, 256], stddev=0.01))  # was[3, 3, 128, 256]
        self.hp_conv3_biases = tf.Variable(tf.random.normal(shape=[256]))
        # Conv layer
        #[patch_size, patch_size, depth, depth]
        self.hp_conv4_weights = tf.Variable(tf.random.truncated_normal([3, 3, 256, 512], stddev=0.01))  # was[3, 3, 128, 256]
        self.hp_conv4_biases = tf.Variable(tf.random.normal(shape=[512]))

        # Dense layer
        #[ 5*5 * previous_layer_out , num_hidden] wd1
        # here 5*5 is the size of the image after pool reduction (divide by
        # half 3 times)
        self.hp_dense1_weights = tf.Variable(tf.random.truncated_normal([10*10*512, 4096], stddev=0.01))  # was [5*5*256, 1024]
        self.hp_dense1_biases = tf.Variable(tf.random.normal(shape=[4096]))
        # Dense layer
        #[ , num_hidden] wd2
        self.hp_dense2_weights = tf.Variable(tf.random.truncated_normal([4096, 4096], stddev=0.01))
        self.hp_dense2_biases = tf.Variable(tf.random.normal(shape=[4096]))
        # Output layer
        self.hp_out_weights = tf.Variable(tf.random.truncated_normal([4096, num_labels_pitch], stddev=0.01))
        self.hp_out_biases = tf.Variable(tf.random.normal(shape=[num_labels_pitch]))
        ...
        def model(data, _dropout=1.0):
            ...
            # Convolution Layer 1
            conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, filters=self.hp_conv1_weights,
            ...
            # Convolution Layer 2
            conv2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm1, filters=self.hp_conv2_weights,
            ...
            # Convolution Layer 3
            conv3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm2, filters=self.hp_conv3_weights,
            ...
            # Convolution Layer 4
            conv4 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv3, filters=self.hp_conv4_weights,
