...
def main():            
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.compat.v1.get_variable("conv1_12d_w", [3, 3, num_channels, 16], initializer=tf_initializer)
            ...
            dense1_weights = tf.compat.v1.get_variable("dense1_12d_w", [6 * 6 * 16, 16], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random.normal(shape=[16]), name="dense1_12d_b")
            # Output layer
            layer_out_weights = tf.compat.v1.get_variable("out_12d_w", [16, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random.normal(shape=[num_labels]), name="out_12d_b")
