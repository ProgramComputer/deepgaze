...
def main():            
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.compat.v1.get_variable("conv1_48d_w", [5, 5, num_channels, 64], initializer=tf_initializer)
            conv1_biases = tf.Variable(tf.zeros([64]), name="conv1_48d_b")
            # Conv layer
            # [patch_size, patch_size, num_channels, depth]
            conv2_weights = tf.compat.v1.get_variable("conv2_48d_w", [5, 5, 64, 64], initializer=tf_initializer)
            ...
            dense1_weights = tf.compat.v1.get_variable("dense1_48d_w", [12 * 12 * 64, 256], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random.normal(shape=[256]), name="dense1_48d_b")
            # Output layer
            layer_out_weights = tf.compat.v1.get_variable("out_48d_w", [256, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random.normal(shape=[num_labels]), name="out_48d_b")
            ...
            def model(data, _dropout=1.0):
                ...
                # Max Pooling (down-sampling)
                pool2 = tf.nn.max_pool2d(input=conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
