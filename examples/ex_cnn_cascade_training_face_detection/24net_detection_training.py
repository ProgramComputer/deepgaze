...
def main():  
        ...
        with graph.as_default():
            ...
            conv1_weights = tf.compat.v1.get_variable("conv1_24d_w", [5, 5, num_channels, 64], initializer=tf_initializer)
            ...
            dense1_weights = tf.compat.v1.get_variable("dense1_24d_w", [12 * 12 * 64, 128], initializer=tf_initializer)
            dense1_biases = tf.Variable(tf.random.normal(shape=[128]), name="dense1_24d_b")
            # Output layer
            layer_out_weights = tf.compat.v1.get_variable("out_24d_w", [128, num_labels], initializer=tf_initializer)
            layer_out_biases = tf.Variable(tf.random.normal(shape=[num_labels]), name="out_24d_b")
